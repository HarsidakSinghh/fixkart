...
export default function OrderHistoryTable({ initialOrders }: { initialOrders: Order[] }) {
    ...
    const [loadingVendorInvId, setLoadingVendorInvId] = useState<string | null>(null);
    ...
    return (
        <div className="rounded-md border bg-white shadow-sm">
            <Table>
                ...
                <TableBody>
                    {initialOrders.length === 0 ? (
                        <TableRow><TableCell colSpan={7} className="text-center h-24 text-muted-foreground">No order history found</TableCell></TableRow>
                    ) : (
                            ...
                            <TableRow key={order.id}>
                                ...
                                <TableCell>
                                    <div className="flex gap-2">
                                        <Button
                                            variant="outline" size="icon" className="h-8 w-8 text-blue-600 border-blue-200 bg-blue-50/50"
                                            onClick={() => handleViewCustomer(order.userId || order.id)}
                                        ...
                                        <Button
                                            variant="outline" size="icon" className="h-8 w-8 text-purple-600 border-purple-200 bg-purple-50/50"
                                            onClick={() => handleViewVendorStandalone(order.vendorId)}
                                            title="View Vendor"
                                        >
                                        ...
                                        <Button
                                            variant="outline" size="icon" className="h-8 w-8 text-gray-700 border-gray-300 bg-gray-100 hover:bg-gray-200"
                                        ...
                                        {/* Transport Slip Button */}
                                        <Button
                                            variant="outline" size="icon"
                                            className={`h-8 w-8 ${order.transportSlipUrl ? 'text-orange-600 border-orange-200 bg-orange-50' : 'opacity-50'}`}
                                            onClick={() => order.transportSlipUrl ? window.open(order.transportSlipUrl, '_blank') : null}
                                            disabled={!order.transportSlipUrl}
                                            title={order.transportSlipUrl ? "View Transport Slip" : "Slip not uploaded"}
                                        >
                                        ...
                                        {order.purchaseOrders && order.purchaseOrders.length > 0 ? (
                                            ...
                                            >
                                                {loadingPoId === order.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <ScrollText className="h-4 w-4" />}
                                            </Button>
                                        )}
            ...
            {/* --- 1. CUSTOMER PROFILE MODAL (Detailed) --- */}
            ...
            {/* --- 2. VENDOR MODAL (Detailed - Updated) --- */}
                                ...
                                <div className="flex gap-4 items-start">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {selectedVendor.gpsLat ? (
                                            <Link
                                                href={`https://www.google.com/maps/search/?api=1&query=$${selectedVendor.gpsLat},${selectedVendor.gpsLng}`}
                                                target="_blank"
                                                className="inline-flex items-center gap-2 bg-blue-600 text-white px-3 py-2 rounded hover:bg-blue-700 text-xs"
                                            >
            ...
            {/* --- 3. FULL ORDER DETAILS MODAL (Keep Existing structure with nice summary) --- */}
            <Dialog open={!!viewOrder} onOpenChange={() => setViewOrder(null)}>
                <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto bg-gray-50/50 p-0 gap-0">
                    <DialogHeader className="p-6 bg-white border-b">
                        <div className="flex items-center justify-between">
                            <div>
                                <DialogTitle className="text-xl font-bold">Order #{viewOrder?.id.slice(-6).toUpperCase()}</DialogTitle>
                                <DialogDescription className="mt-1">Placed on {viewOrder && new Date(viewOrder.createdAt).toLocaleDateString()}</DialogDescription>
                            </div>
                            {viewOrder && <Badge variant="outline">{viewOrder.status}</Badge>}
                        </div>
                    </DialogHeader>
                    {viewOrder && (
                        <div className="p-6 space-y-6">
                                    ...
                                    <TableBody>
                                        {viewOrder.items.map((item, idx) => (
                                            <TableRow key={idx}>
                                                ...
                                                <TableCell className="text-right font-bold">₹{(item.price * item.quantity).toFixed(2)}</TableCell>
                                            </TableRow>
                                        ))}
                                    </TableBody>
                                </Table>
                            ...
                            <div className="border-t-4 border-blue-100 pt-6">
                                <h4 className="font-bold flex items-center gap-2 mb-4 text-blue-800 text-sm uppercase tracking-wide"><Store className="h-4 w-4" /> Fulfilled By</h4>
                                {viewOrderVendor ? (
                                    <div className="bg-white p-4 rounded border grid md:grid-cols-2 gap-4">
                                        <div>
                                            <p className="font-bold">{viewOrderVendor.companyName}</p>
                                            <p className="text-sm text-gray-600">{viewOrderVendor.fullName}</p>
                                            <p className="text-xs text-gray-400 mt-1">{viewOrderVendor.email}</p>
                                        </div>
                                        <div className="text-sm space-y-1">
                                            <p><span className="text-gray-500">Phone:</span> {viewOrderVendor.phone}</p>
                                            <p><span className="text-gray-500">City:</span> {viewOrderVendor.city}</p>
                                        </div>
                                    </div>
                                ) : <div className="p-4 text-gray-500">Loading Vendor...</div>}
                            </div>
                        </div>
                    )}
                </DialogContent>
