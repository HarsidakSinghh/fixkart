...
import { FileText, MapPin, ExternalLink, Building, CreditCard, Phone, ShieldCheck } from "lucide-react";
        ...
        <div className="space-y-6 max-w-7xl mx-auto p-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-lg border shadow-sm">
                <div className="flex items-center gap-4">
                    {customer.ownerPhotoUrl ? (
                        <img src={customer.ownerPhotoUrl} alt="Owner" className="w-16 h-16 rounded-full object-cover border-2 border-gray-100" />
                    ) : (
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">{customer.companyName || customer.fullName}</h1>
                ...
                <div className="flex items-center gap-4">
                    <Badge
                        variant={customer.status === "APPROVED" ? "default" : customer.status === "SUSPENDED" ? "destructive" : "secondary"}
                        className="text-base px-4 py-1"
                    >
                        {customer.status}
                    </Badge>
                    <Button variant="outline" asChild>
                        <Link href="/admin/onboarded-customers">Back to List</Link>
                    </Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3 pt-4">
                        ...
                        <div className="flex justify-between border-b pb-2">
                            ...
                            <span className="text-sm">{customer.businessType || "N/A"}</span>
                        </div>
                        <div className="flex justify-between border-b pb-2">
                            <span className="font-semibold text-sm">Years in Business:</span>
                            <span className="text-sm">{customer.yearsInBusiness || "N/A"}</span>
                        </div>
                        <div className="flex justify-between border-b pb-2">
                            <span className="font-semibold text-sm">GST Number:</span>
                            <span className="text-sm font-mono">{customer.gstNumber || "N/A"}</span>
                        </div>
                        <div className="flex justify-between border-b pb-2">
                            <span className="font-semibold text-sm">Trade License:</span>
                            <span className="text-sm">{customer.tradeLicense || "N/A"}</span>
                        </div>
                    ...
                    <CardContent className="space-y-3 pt-4">
                        ...
                        <div className="space-y-3 mt-2">
                            <div>
                                ...
                                {customer.aadharCardUrl ? (
                                    <a href={customer.aadharCardUrl} target="_blank" className="block w-full h-24 rounded-md border overflow-hidden hover:opacity-90 transition bg-gray-50 flex items-center justify-center relative group">
                                        <img src={customer.aadharCardUrl} alt="Aadhar" className="max-w-full max-h-full object-contain" />
                            ...
                            <div>
                                ...
                                {customer.panCardUrl ? (
                                    <a href={customer.panCardUrl} target="_blank" className="block w-full h-24 rounded-md border overflow-hidden hover:opacity-90 transition bg-gray-50 flex items-center justify-center relative group">
                                        <img src={customer.panCardUrl} alt="PAN" className="max-w-full max-h-full object-contain" />
                                        <div className="absolute inset-0 bg-black/50 hidden group-hover:flex items-center justify-center text-white text-xs font-bold">View Full</div>
                                    </a>
                                ) : (
                                    <span className="text-xs text-red-500 italic">Not Uploaded</span>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3 pt-4">
                        <div className="flex justify-between border-b pb-2">
                            ...
                            <span className="text-sm">{customer.bankName}</span>
                        </div>
                        <div className="flex justify-between border-b pb-2">
                            <span className="font-semibold text-sm">Account Holder:</span>
                            <span className="text-sm">{customer.accountHolder}</span>
                        </div>
                        <div className="flex justify-between border-b pb-2">
                        ...
                        <div className="flex justify-between border-b pb-2">
                            ...
                            <span className="text-sm font-mono">{customer.ifscCode}</span>
                        </div>
                        {customer.cancelledChequeUrl && (
                            <div className="pt-2">
                                <span className="font-semibold text-sm block mb-1">Cancelled Cheque:</span>
                                <a href={customer.cancelledChequeUrl} target="_blank" className="block w-full h-32 rounded-md border overflow-hidden hover:opacity-90 transition bg-gray-50 flex items-center justify-center">
                                    <img src={customer.cancelledChequeUrl} alt="Cheque" className="max-w-full max-h-full object-contain" />
                                </a>
                            </div>
                        )}
                    </CardContent>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4 pt-4">
                        {customer.locationPhotoUrl ? (
                            <div className="relative w-full h-40 rounded-lg overflow-hidden border bg-gray-50 flex items-center justify-center">
                                <img src={customer.locationPhotoUrl} alt="Shop Location" className="max-w-full max-h-full object-contain" />
                            </div>
                        ) : (
                        ...
                        <div className="flex items-center justify-between border-t pt-3">
                            <div>
                                ...
                                <p className="text-sm font-mono">{customer.gpsLat ?? "N/A"}, {customer.gpsLng ?? "N/A"}</p>
                            </div>
                            {customer.gpsLat && customer.gpsLng && (
                                <Button size="sm" variant="outline" className="gap-2 h-8" asChild>
                                    <a href={`https://www.google.com/maps/search/?api=1&query=$${customer.gpsLat},${customer.gpsLng}`} target="_blank">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4 pt-4">
                        <div className="bg-orange-50 p-3 rounded-md border border-orange-100">
                            ...
                            <p className="text-sm font-semibold">{customer.backup1Name || "N/A"}</p>
                            <p className="text-sm text-gray-600">{customer.backup1Phone || "N/A"}</p>
                            {customer.backup1IdUrl && (
                                <a href={customer.backup1IdUrl} target="_blank" className="text-xs text-blue-600 hover:underline mt-1 block">View ID Proof</a>
                            )}
                        </div>

                        <div className="bg-orange-50 p-3 rounded-md border border-orange-100">
                            <p className="text-xs font-bold text-orange-800 uppercase mb-1">Contact Person 2</p>
                            <p className="text-sm font-semibold">{customer.backup2Name || "N/A"}</p>
                            <p className="text-sm text-gray-600">{customer.backup2Phone || "N/A"}</p>
                            {customer.backup2IdUrl && (
                                <a href={customer.backup2IdUrl} target="_blank" className="text-xs text-blue-600 hover:underline mt-1 block">View ID Proof</a>
                            )}
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="md:col-span-2 lg:col-span-3">
                    ...
                    <CardContent className="pt-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-3">
                            <DocRow label="Aadhar Card" url={customer.aadharCardUrl} />
                            <DocRow label="PAN Card" url={customer.panCardUrl} />
                            <DocRow label="GST Certificate" url={customer.gstCertificateUrl} />
                            <DocRow label="MSME Certificate" url={customer.msmeCertificateUrl} />
                            <DocRow label="Cancelled Cheque" url={customer.cancelledChequeUrl} />
                            <DocRow label="Shop/Location Photo" url={customer.locationPhotoUrl} />
                            <DocRow label="Owner Photo" url={customer.ownerPhotoUrl} />
                            <DocRow label="Backup Contact 1 ID" url={customer.backup1IdUrl} />
                            <DocRow label="Backup Contact 2 ID" url={customer.backup2IdUrl} />
                            <DocRow label="Business Proof" url={customer.businessProofUrl} />
                            <DocRow label="ID Proof" url={customer.idProofUrl} />
                        </div>
                    </CardContent>
                </Card>
