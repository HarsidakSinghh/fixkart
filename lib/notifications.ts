...
export async function sendNotification(type: NotificationType, data: NotificationData) {
    ...
    if (!recipientEmail) {
        console.log(`⚠️ Email skipped: No recipient email for [${type} - ${data.name}]`);
        return;
    }
...
function getSubject(type: NotificationType, data: NotificationData) {
    ...
    switch (type) {
        ...
        case "ORDER_APPROVED": return `Order #${orderId} Approved and Processing!`;
...
function getMessageText(type: NotificationType, data: NotificationData) {
    ...
    switch (type) {
        ...
        case "ORDER_APPROVED":
            return `Hi ${data.name},<br><br>Good news! Your order #${orderId} has been <strong>APPROVED</strong> by our team.<br>We are now processing it and will notify you when it ships.`;
